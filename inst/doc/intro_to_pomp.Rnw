...
In this function, \code{x}, \code{times}, and \code{params} are as they were above.
Notice that we give the returned array, \code{y}, rownames.
<<>>=
...
<<>>=
  bvnorm.dmeasure <- function (y, x, times, params, log = FALSE, ...) {
    ...
    array(f,dim=c(ncol(x),length(times)))
  }
@ 
...
<<>>=
 tic <- Sys.time()
 ou2 <- simulate(ou2,params=true.p,nsim=1000)
 toc <- Sys.time()
 print(toc-tic)
 ou2 <- ou2[[1]]
@ 
...
dim(x)
@ 
Note that the dimensions of \code{x} are \verb+nvars x nsims x ntimes+, where \code{nvars} is the number of state variables, \code{nsims} is the number of simulated trajectories (which is the number of columns in the \code{params} and \code{xstart} matrices), and \code{ntimes} is the length of the \code{times} argument.
...
b <- diag(1,2)
sigma <- matrix(c(true.p['sigma.1'],p['sigma.2'],0,true.p['sigma.3']),2,2)
